...
firepad.Firepad = (function(global) {
  ...
  /**
  Assists in creating a dialog for the header
  */
  Firepad.prototype.makeHeaderDialog_ = function(){
    this.makeDialog_('header-timestamp', 'Insert text for the header', true);
  }

  Firepad.prototype.makeDialog_ = function(id, placeholder, header_flag) {
   var self = this;
   ...
   var cb = function() {
     ...
     if (header_flag){
       // Insert into info with text content and current time
       // TODO - Set timestamp
       self.insertEntity(id, {timestamp: '0:00', textContent: src})
     } else if (src !== null){
  ...
  Firepad.prototype.addToolbar_ = function() {
    ...
    this.toolbar.on('insert-header-timestamp', this.makeHeaderDialog_, this);
